...
export default function StreamMonitor({ senderAddress }: Props) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setWithdrawTx(tx);
      const updated = await fetchStream(senderAddress, publicKey!.toString());
      setStream(updated);
    } catch (e: any) {
  ...
  if (!stream) return <p className="text-gray-400 text-sm p-4">Loading stream...</p>;

  return (
    <div className="w-full max-w-lg mx-auto p-6 bg-gray-900 rounded-2xl">
      <h2 className="text-xl font-bold text-white mb-6">Live Stream</h2>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="p-3 bg-gray-800 rounded-xl">
          ...
          <p className="text-lg font-bold text-white">{earned.toFixed(4)}</p>
          <p className="text-xs text-gray-500">AUDD</p>
        </div>
        <div className="p-3 bg-gray-800 rounded-xl">
          <p className="text-xs text-gray-500">Claimable</p>
          <p className="text-lg font-bold text-blue-400">{claimable.toFixed(4)}</p>
          <p className="text-xs text-gray-500">AUDD</p>
        </div>
        <div className="p-3 bg-gray-800 rounded-xl">
          <p className="text-xs text-gray-500">Withdrawn</p>
          <p className="text-lg font-bold text-green-400">{withdrawn.toFixed(4)}</p>
          <p className="text-xs text-gray-500">AUDD</p>
        </div>
        <div className="p-3 bg-gray-800 rounded-xl">
  ...
  <div className="p-3 bg-gray-800 rounded-xl mb-3">
    ...
    <p className="text-xs text-blue-400 break-all cursor-pointer hover:underline"
      onClick={() => window.open(`https://explorer.solana.com/tx/${withdrawTx}?cluster=devnet`, '_blank')}>
      View on Explorer
    </p>
  </div>
)}
      ...
      <button
        ...
        className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white font-semibold rounded-xl transition"
      >
        {loading
          ? "Withdrawing..."
          : claimable > 0
      ...
      {stream.isCancelled && (
        <p className="text-center text-gray-500 text-sm mt-3">Stream cancelled</p>
      )}
    </div>
  );
}